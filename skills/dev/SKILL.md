...
3. `<artifacts_dir>/<task-id>/` ディレクトリを作成する
4. タスク内容を `<artifacts_dir>/<task-id>/task.md` に保存する
5. 設定内容を `<artifacts_dir>/<task-id>/config-snapshot.md` に保存する（デバッグ用）

### Step 1: 情報収集 (Research)
1. `researcher` AgentをSubAgentとして起動する
   - 入力: `<artifacts_dir>/<task-id>/task.md`
   - 出力: `<artifacts_dir>/<task-id>/research.md`
2. `reviewer` AgentにSubAgentとしてレビューを依頼する
   - 入力: `<artifacts_dir>/<task-id>/research.md`
   - レビュー観点: 「調査は十分か、見落としている既存コードや依存関係はないか」
   - 出力: `<artifacts_dir>/<task-id>/research-review.md`
3. FAILなら修正を依頼する（設定の max_retries.research 回まで）

### Step 2: 設計 (Design)
1. `designer` AgentをSubAgentとして起動する
   - 入力: task.md + research.md
   - 出力: `<artifacts_dir>/<task-id>/design.md`
2. `reviewer` AgentにSubAgentとしてレビューを依頼する
   - レビュー観点: 「設計はタスク要件を満たしているか、既存アーキテクチャと整合性があるか」
   - 出力: `<artifacts_dir>/<task-id>/design-review.md`
...
1. `tester` AgentをSubAgentとして起動する
   ...
   - 出力: `<artifacts_dir>/<task-id>/test-report.md`
2. `reviewer` AgentにSubAgentとしてテストレビューを依頼する
   - レビュー観点: 「テストケースは要件を正しく検証しているか、網羅性は十分か」
   - 出力: `<artifacts_dir>/<task-id>/test-review.md`
...
1. `implementer` AgentをSubAgentとして起動する
   ...
   - 出力: `<artifacts_dir>/<task-id>/implement-report.md`
2. `reviewer` AgentにSubAgentとしてレビューを依頼する
   - レビュー観点: 「設計通りか、テスト全パスか、規約違反ないか」
   - 出力: `<artifacts_dir>/<task-id>/implement-review.md`
3. FAILの場合:
...
1. `pr-creator` AgentをSubAgentとして起動する
   - 入力: `<artifacts_dir>/<task-id>/` 配下の全Artifact + 設定のpr.*
   - 実行: git commit + git push + gh pr create
   - 出力: `<artifacts_dir>/<task-id>/pr-url.txt`

## エラーハンドリング
- レビュー上限到達でもFAILの場合: 現状態でcommitし、PR本文に「⚠️ レビュー未通過: <ステップ名>」を明記してDraft PR作成
- SubAgent異常終了: `<artifacts_dir>/<task-id>/error.md` にエラー記録、可能な範囲でPR作成
- 設定ファイル不在: デフォルト値で続行（エラーにしない）

## Artifact
- `<artifacts_dir>/` はgitignore対象
- PR作成後も残す（デバッグ用）
- worktree削除時に消える
